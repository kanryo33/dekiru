...
def parse_page(page, side):
    ...
    for jy0, jy1, jtoks in jp_clusters:
        ...
        engs = [w for w in eng_toks if (jy0-9) <= w[1] <= (jy1+9)]
    ...
    for pno in range(len(doc)):
        ...
        for w in words:
            if 'ことば' in w[4] and w[0] < 60:
                markers.append(('kotoba', pno, w[1]))
        for w in words:
            if re.search(r'第\s*\d+\s*課', w[4].replace(' ', '')) and w[0] < 300 and w[1] < 60:
                markers.append(('lesson', pno, w[1], w[4].replace(' ', '')))
    print('markers:', markers[:40])
